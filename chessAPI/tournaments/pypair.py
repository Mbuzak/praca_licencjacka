class Swiss:
    ...
    def __init__(self, players, games=[], next_round=1):
        ...
        for game in sorted_games:
            ...
            info['score'] += float(game['player_score'])
    ...
    def order_players(self, players):
        ...
        # sorted_players = sorted(sorted_players, reverse=True, key=lambda player: player['PZSzach'])
        sorted_players = sorted(players, reverse=True, key=lambda player: player['title'])
        sorted_players = sorted(sorted_players, reverse=True, key=lambda player: player['rating'])
        sorted_players = sorted(sorted_players, reverse=True,
                                key=lambda player: self.player_info[player['name']]['score'])
        return sorted_players

    def pair_first_round(self):
        sorted_players = self.order_players(self.players)
        match_count = int(len(self.players) / 2)
        for i in range(match_count):
            self.pairs.append([sorted_players[i], sorted_players[match_count + i]])

        if len(self.players) % 2 == 1:
            self.pairs.append([sorted_players[match_count * 2], None])
    ...
    def find_possible_opponents(self, current_player, group, skip_color_pref=False):
        ...
        rest = [player for player in rest if 'pair' not in player or player['pair'] is False]
        if len(rest) == 0:
            return []
