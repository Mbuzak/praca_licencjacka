...
from accounts.models import PROVINCE_CHOICES
...
class TournamentFilter(django_filters.FilterSet):
    ...
    province_filter = django_filters.ChoiceFilter(label='Województwo', choices=PROVINCE_CHOICES, method='province_filtering')

    class Meta:
        model = To
        fields = []

    def province_filtering(self, queryset, name, value):
        id_list = [x.id for x in queryset.all() if x.get_province().lower() == value]
