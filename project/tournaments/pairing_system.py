...
def create_pairing(pk):
    ...
    for i in range(0 + odd_players[1], len(pairing)):
        Match(round_id=last_round.id, chessboard=i + 1 - odd_players[1],
              white_id=players[pairing[i][0] - 1].person.id, black_id=players[pairing[i][1] - 1].person.id).save()
...
def update_results(pk):
    ...
    for member in TournamentMember.objects.filter(tournament_id=pk):
        white_points = sum([results[x.white_result] for x in matches.filter(white_id=member.person.id)])
        black_points = sum([results[x.black_result] for x in matches.filter(black_id=member.person.id)])
...
def obtain_rating(tournament_id, person_id):
    rating_sum = 0
    rounds = [x.id for x in Round.objects.filter(tournament_id=tournament_id)]
    matches = Match.objects.filter(Q(white_id=person_id) | Q(black_id=person_id), round_id__in=rounds)
    for match in matches:
        if match.white.id == person_id:
            rating_sum += match.black.get_polish_rating()
        else:
            rating_sum += match.white.get_polish_rating()

    match_count = len(matches)
    person_rating = Account.objects.get(pk=person_id).get_polish_rating()
    points = TournamentMember.objects.get(tournament_id=tournament_id, person_id=person_id).points
...
def update_categories(tournament):
    for member in TournamentMember.objects.filter(tournament.id):
        acc = Account.objects.get(pk=member.id)
        begin = TITLE_TUPLE.index(acc.category) + 1
        end = len(TITLE_TUPLE) - 1

        rating = obtain_rating(tournament.id, acc.id)
